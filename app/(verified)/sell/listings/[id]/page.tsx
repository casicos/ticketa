...
export default async function SellListingDetailPage({
  ...
  const { data } = await supabase
    .from('listing')
    .select(
      'id, status, seller_id, quantity_offered, quantity_remaining, unit_price, gross_amount, commission_total, seller_net_amount, submitted_at, purchased_at, handed_over_at, received_at, verified_at, shipped_at, completed_at, cancelled_at, cancel_reason, admin_memo, sku:sku_id(brand, denomination, display_name, thumbnail_url)',
    )
    .eq('id', id)
