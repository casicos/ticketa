-- 0038_fix_purchase_format.sql
--
-- 0034/0037 의 format() 에 잘못된 spec(`%`) 가 들어가 RPC 호출 시 예외 발생.
-- `format('매입: listing=% qty=%s', ...)` → `format('매입: listing=%s qty=%s', ...)` 로 수정.
...
begin
  ...
  perform public.debit_mileage(
    ...
    format('매입: listing=%s qty=%s', p_listing, p_qty)
  );
