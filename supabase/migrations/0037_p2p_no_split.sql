-- 0037_p2p_no_split.sql
--
-- purchase_listing 분기 재정의:
--   - 에이전트(pre_verified=true) 매물: split-at-purchase — 자식 listing 생성, 부모 quantity_remaining 차감
--   - P2P (pre_verified=false) 매물:    no-split — 부모 자체를 매입 처리 (기존 동작 유지)
--     * 자식 listing 없이 buyer_id/status/commission 등을 부모에 직접 세팅
--     * /sell/listings·/admin/intake 의 기존 단일-listing 흐름이 그대로 동작
...
begin
  ...
  if v_parent.status <> 'submitted' then raise exception 'INVALID_STATE: %', v_parent.status; end if;
  if v_parent.seller_id = p_buyer then raise exception 'SELF_PURCHASE_FORBIDDEN'; end if;
  ...
  if v_balance is null or v_balance < v_gross then
    return jsonb_build_object(
      ...
      'needed', v_gross, 'balance', coalesce(v_balance, 0),
  ...
  if v_parent.pre_verified then
    ...
    insert into public.listing(
      ...
      v_child_status, v_now, v_now, v_now, v_now,
      p_buyer, true,
    ...
    -- 부모의 agent_listing_created 이벤트에서 inventory_id 가져와 자식에도 복제
    ...
    if v_parent_inv_id is not null then
      ...
      values (p_buyer, 'listing', v_child_id, 'agent_listing_created', v_child_status,
              jsonb_build_object(
    ...
    insert into public.audit_events(
      ...
      p_buyer, 'listing', v_child_id, 'status_change:verified',
      'submitted', 'verified',
      jsonb_build_object(
        'parent_listing_id', p_listing,
        'gross', v_gross, 'commission', v_commission, 'seller_net', v_seller_net,
        'pre_verified', true, 'qty', p_qty
      )
    );
    ...
    -- P2P 매물: split 없이 부모를 직접 매입 처리 (기존 동작 그대로)
    ...
    insert into public.audit_events(
      ...
      p_buyer, 'listing', p_listing, 'status_change:purchased',
      'submitted', v_to_state,
      jsonb_build_object(
        'gross', v_gross, 'commission', v_commission, 'seller_net', v_seller_net,
        'pre_verified', false, 'qty', p_qty
      )
    );
