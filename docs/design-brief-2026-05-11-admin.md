...
2. **2-column 메인**:
   - 좌 (8/12) **"새 위탁 입고" 폼 카드** — 에이전트 select (드롭다운에 상점명·@username 함께) + SKU select (브랜드별 그룹) + 수량 (± 컨트롤 + 빠른 버튼: 10/30/50/100) + 위탁 단가 (₩ 입력, step 100) + 우측 미리보기 패널 (적재 후 상태 예고: 기존 행 결합 vs 신규 행). 하단에 "총 적재가" 강조.
   - 우 (4/12) **에이전트 카드 리스트** — 각 에이전트별 현재 보유량 요약 (상위 5명, 보유/판매중/총위탁가)
3. **하단 "최근 적재 내역" 테이블** — 에이전트 / SKU / 보유·판매중 / 단가 / 총 위탁가 / 갱신 시각. 행 클릭 시 해당 에이전트의 전체 inventory drill-in (사이드 시트).
